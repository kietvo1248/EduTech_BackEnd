...
import { LearningPathService, LearningPathNode, LessonUnlockResult } from './learning-path.service';
...
export class LearningPathController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get personalized learning path based on user grade level' })
  @ApiResponse({ status: 200, description: 'Learning path retrieved successfully' })
  async getLearningPath(@CurrentUser() user: User): Promise<LearningPathNode[]> {
  ...
  @ApiOperation({ summary: 'Check if lesson can be unlocked based on prerequisites' })
  @ApiResponse({ status: 200, description: 'Unlock status checked successfully' })
