...
export class LearningPathService {
  ...
  async getLearningPath(
    ...
    for (const course of courses) {
      ...
      const chapters = await this.courseService.getChaptersWithLessons(course.id);
      const courseProgress = await this.calculateCourseProgress(userId, course.id);
      
      const courseNode: LearningPathNode = {
        id: course.id,
        title: course.title,
        type: 'subject',
        status: courseProgress === 100 ? 'completed' : courseProgress > 0 ? 'in-progress' : 'locked',
        progress: courseProgress,
        children: [],
      };

      for (const chapter of chapters) {
        const chapterProgress = await this.calculateChapterProgress(userId, chapter.id);
        const chapterNode: LearningPathNode = {
          id: chapter.id,
          title: chapter.title,
          type: 'chapter',
          status: chapterProgress === 100 ? 'completed' : chapterProgress > 0 ? 'in-progress' : 'locked',
          progress: chapterProgress,
          children: [],
        };

        // Lấy lessons của chapter theo thứ tự
        const lessons = await this.lessonService.findByChapterIdOrdered(chapter.id);
        
        for (let i = 0; i < lessons.length; i++) {
          const lesson = lessons[i];
          const lessonProgress = await this.lessonProgressService.findByUserAndLesson(userId, lesson.id);
          const prerequisiteCompleted = await this.checkLessonPrerequisite(userId, lesson.id, i);
  ...
  async checkLessonUnlock(userId: string, lessonId: string): Promise<LessonUnlockResult> {
    ...
    const previousLesson = await this.lessonService.findPreviousLesson(lessonId);
    if (!previousLesson) {
    ...
    const videoCompleted = await this.checkVideoCompleted(userId, previousLesson.id);
    ...
    if (!videoCompleted) {
      return { success: false, message: 'Bạn cần xem xong video bài trước để mở khóa bài này' };
    }

    if (!quizPassed) {
      return { success: false, message: 'Bạn cần đạt điểm > 80% trong bài tập trước để mở khóa bài này' };
    }
  ...
  private async calculateCourseProgress(userId: string, courseId: string): Promise<number> {
    ...
    for (const lesson of lessons) {
      const progress = await this.lessonProgressService.findByUserAndLesson(userId, lesson.id);
  ...
  private async calculateChapterProgress(userId: string, chapterId: string): Promise<number> {
  ...
  private async checkLessonPrerequisite(userId: string, lessonId: string, lessonIndex: number): Promise<boolean> {
  ...
  private getLessonStatus(lessonProgress: any, prerequisiteCompleted: boolean): 'locked' | 'in-progress' | 'completed' {
  ...
  private async checkVideoCompleted(userId: string, lessonId: string): Promise<boolean> {
    const progress = await this.lessonProgressService.findByUserAndLesson(userId, lessonId);
    return progress?.videoWatched || false;
  }

  private async checkQuizPassed(userId: string, quizId: string, passingScore: number): Promise<boolean> {
    if (!quizId) return true; // Nếu không có quiz thì coi như đã pass

    const bestAttempt = await this.quizAttemptService.findBestAttemptByUserAndQuiz(userId, quizId);
