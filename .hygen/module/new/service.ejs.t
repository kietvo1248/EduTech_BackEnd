...
import { <%= h.changeCase.pascal(name) %>RepositoryAbstract } from '../infrastructure/persistence/document/repositories/<%= name %>.repository.abstract';
...
export class <%= h.changeCase.pascal(plural) %>Service {
  constructor(
    private readonly <%= h.changeCase.camel(name) %>Repository: <%= h.changeCase.pascal(name) %>RepositoryAbstract,
  ) {}
